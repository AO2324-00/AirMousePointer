...
class HandState:
    ...
    def isRecognizing(self, hands_landmarks, pose_landmarks):
        ...
        is_orthogonal = [65 < hand_angle[0] < 110, 65 < hand_angle[1] < 110]
    ...
    def calcPosition(self, hands_landmarks, pose_landmarks):
        ...
        if calcRelativeHandsPosition(hands_landmarks, pose_landmarks):
            p0 = vector.Vector3D({'x': p0.x, 'y': p0.y, 'z': p0.z + pose_landmarks.pose_landmarks.landmark[15].z})
            p1 = vector.Vector3D({'x': p1.x, 'y': p1.y, 'z': p1.z + pose_landmarks.pose_landmarks.landmark[16].z})
        else:
